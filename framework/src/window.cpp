    ...
    : m_glVersion(glVersion)
{
    ...
    glfwSetWindowPos(m_pWindow, 80, 80);
}
...
void Window::setBorderedFullScreen()
{
    // set to full screen
    GLFWmonitor* monitor = glfwGetPrimaryMonitor();
    const GLFWvidmode* mode = glfwGetVideoMode(monitor);
    // set at center of screen
    glfwSetWindowMonitor(m_pWindow, NULL, (mode->width - m_windowSize.x) / 2, (mode->height - m_windowSize.y) / 2 + 10, m_windowSize.x, m_windowSize.y, mode->refreshRate);
}
